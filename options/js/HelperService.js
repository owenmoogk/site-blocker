(function (app) {
	var HelperService = ['$rootScope', function ($rootScope) {
		var Helper = {};

		Helper.md5 = function (string) {
			if (!string) string = '';
		...
		Helper.getKey = function (key, jsonParse, defaultVal) {
		...
		Helper.getExtraHour = function () {
			var hour_config_extra = localStorage.getItem('hour_config_extra');
			if (hour_config_extra) {
				hour_config_extra = JSON.parse(hour_config_extra);
			} else {
				hour_config_extra = [];
			}
			return hour_config_extra;
		}
